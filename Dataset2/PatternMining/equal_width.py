...
plt.tight_layout()
savefig("images/equal_width/nr_patterns_support.png", dpi=300)
show()
...
def plot_top_rules(rules: DataFrame, metric: str, per_metric: str) -> None:
    ...
    ax.text(0, 0, text)
    plt.tight_layout()
    savefig(f'images/equal_width/TOP_10_per_Min{per_metric}-{metric}.png', dpi=300)
    show()
...
def analyse_per_metric(rules: DataFrame, metric: str, metric_values: list) -> list:
    ...
    plt.tight_layout()
    savefig(f"images/equal_width/{metric}_1.png", dpi=300)
    show()
...
plt.tight_layout()
savefig(f"images/equal_width/Nr_rules_Support.png", dpi=300)
...
plot_line(var_min_conf, nr_rules_cf, title='Nr Rules x Confidence', xlabel='confidence', ylabel='Nr Rules',
          percentage=False)
plt.tight_layout()
savefig(f"images/equal_width/Nr_rules_confidence.png", dpi=300)
