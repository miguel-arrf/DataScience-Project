...
data = read_csv('../../data/set2_equal_frequency.csv').sample(frac=0.05)
...
MIN_SUP: float = 0.03
var_min_sup = [0.05, 0.045, 0.040, 0.035, 0.030]
print(var_min_sup)
...
plt.tight_layout()
savefig("images/equal_frequency/nr_patterns_support.png", dpi=300)
show()
...
def plot_top_rules(rules: DataFrame, metric: str, per_metric: str) -> None:
    ...
    ax.text(0, 0, text)
    plt.tight_layout()
    savefig(f'images/equal_frequency/TOP_10_per_Min{per_metric}-{metric}.png', dpi=300)
    show()
...
def analyse_per_metric(rules: DataFrame, metric: str, metric_values: list) -> list:
    ...
    plt.tight_layout()
    savefig(f"images/equal_frequency/{metric}_1.png", dpi=300)
    show()
...
plt.tight_layout()
savefig(f"images/equal_frequency/Nr_rules_Support.png", dpi=300)
...
plot_line(var_min_conf, nr_rules_cf, title='Nr Rules x Confidence', xlabel='confidence', ylabel='Nr Rules',
          percentage=False)
plt.tight_layout()
savefig(f"images/equal_frequency/Nr_rules_confidence.png", dpi=300)
