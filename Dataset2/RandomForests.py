    ...
    def __init__(self, trnX, trnY, tstY, tstX, imagePath):
        ...
        for k in range(len(max_depths)):
            ...
            multiple_line_chart(n_estimators, values, ax=axs[0, k], title=f'Random Forests with max_depth={d}',
                                xlabel='nr estimators', ylabel='recall %', percentage=True, extraPercentage=0.0,
                                minExtraPercentage=0.0)
        savefig(f'{imagePath}/random_forests/rf_study.png', dpi=300)
        show()
        print('Best results with depth=%d, %1.2f features and %d estimators, with recall=%1.2f' % (
        ...
        plot_evaluation_results(["Safe", "Danger"], trnY, prd_trn, tstY, prd_tst, extra="Random Forest")
        ...
        f.write(
            'Best results with depth=%d, %1.2f features and %d estimators ==> recall=%1.2f' % (
                best[0], best[1], best[2], last_best))
        f.close()
