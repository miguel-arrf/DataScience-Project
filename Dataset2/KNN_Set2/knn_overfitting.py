...
from Dataset2.NaiveBayes_Set2.balanceData_set2 import BalanceData
...
killed = df.loc[df[var] == 'Danger']
injured = df.loc[df[var] == 'Safe']
...
print("trnx: {}, tstX: {}, trnY:{}, tstY:{}".format(len(trnX), len(tstX), len(trnY), len(tstY)))
...
for coiso in [(trnY, "train", trnX), (tstY, "test", tstX)]:
    ...
    for score in [(accuracy_score, 'accuracy'), (precision_score, 'precision'), (recall_score, 'recall')]:
        ...
        for d in range(len(dist)):
            ...
            print("estamos na distancia: ", dist)

            f = dist[d]
            yvalues = []
            for n in nvalues:
                print("estamos no n value: ", n)
