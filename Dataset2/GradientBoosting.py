...
class GradientBoosting:
    def __init__(self, trnX, trnY, tstY, tstX, imagePath):
        ...
        print('Best results with depth=%d, learning rate=%1.2f and %d estimators, with recall=%1.2f' % (
            best[0], best[1], best[2], last_best))

        # Best model:
        prd_trn = best_model.predict(trnX)
        prd_tst = best_model.predict(tstX)
        plot_evaluation_results(["Safe", "Danger"], trnY, prd_trn, tstY, prd_tst, extra="Gradient Boosting")
        ...
        f.write('Best results with depth=%d, learning rate=%1.2f and %d estimators, with recall=%1.2f' % (
            best[0], best[1], best[2], last_best))
        f.close()
