...
norm_data_zscore.to_csv(f'data/NYC_collisions_tabular_scaled_zscore.csv', index=False)

transf = MinMaxScaler(feature_range=(0, 1), copy=True).fit(df_nr)
tmp = DataFrame(transf.transform(df_nr), index=data.index, columns=numeric_vars)
norm_data_minmax = concat([tmp, df_sb, df_bool], axis=1)
norm_data_minmax.to_csv(f'data/NYC_collisions_tabular_scaled_minmax.csv', index=False)
...
plt.savefig(f'../Scaling_Set1/images/scaling.png')
show()
