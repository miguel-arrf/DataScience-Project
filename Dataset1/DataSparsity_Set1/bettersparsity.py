...
data = data.query('PERSON_INJURY == "Killed"')
print(data.head(2))
...
colors = np.where(data["PERSON_INJURY"] == "Injured", 'red', 'red')
...
for i in range(len(symbolic_vars)):
    ...
    for j in range(i + 1, len(symbolic_vars)):
        ...
        seaborn.stripplot(y=data[var1].astype(str), x=data[var2].astype(str), jitter=0.35, edgecolor='gray', alpha=.25, ax=axs[i, j - 1], palette=colors)
        #seaborn.despine()
print("All coiso")
savefig(f'../DataSparsity_Set1/images/sparsity_study_symbolic_killed.png')
print("Done.")
